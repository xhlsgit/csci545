...
LINEAR_MODEL_VAR_X = 100
LINEAR_MODEL_VAR_Y = 100
...
class Particle(object):
    ...
    def UpdateOdom(self, odom_msg):
        ...
        vx += np.random.normal(0, LINEAR_MODEL_VAR_X)
        vy += np.random.normal(0, LINEAR_MODEL_VAR_Y)
        map_v = RotateBy(np.array([vx,vy],dtype=dtype), self.map_T_particle.rotation)
        self.map_T_particle.translation[0] += dt * map_v[0]
        self.map_T_particle.translation[1] += dt * map_v[1]

        vz = odom_msg.twist.twist.angular.z
        vz += np.random.normal(0, ANGULAR_MODEL_VAR)
        self.map_T_particle.rotation += dt * vz
    ...
    def UpdateScan(self, scan):
        ...
        probs_zt = 1 / np.sqrt(2*np.pi*SENSOR_MODEL_VAR) * \
            np.exp(-np.square(sim_ranges - scan.ranges)/(2*SENSOR_MODEL_VAR))
        self.weight = np.prod(probs_zt)
...
class ParticleFilter(object):
    ...
    def UpdateScan(self, scan_msg):
        ...
        inds_drawed = np.random.choice(len(weights), size=NUM_PARTICLES, p=probs)
        particles_drawed = [copy.deepcopy(self.particles[i]) for i in inds_drawed]
        self.particles = particles_drawed
