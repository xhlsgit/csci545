...
class AdaRRT():
    ...
    def _get_random_sample(self):
        ...
        for dim in range(len(self.joint_lower_limits)):
            s = np.random.uniform(self.joint_lower_limits[dim], self.joint_upper_limits[dim])
            sample.append(s)
        return np.asarray(sample)

    def _get_random_sample_near_goal(self):
        """
        Samples around the goal state

        :return: A vector representing a sampled point arount goal state
        """
        sample = []
        gs = self.goal.state
        rad = 0.1
        for dim in range(len(self.joint_lower_limits)):
            s = np.random.uniform(gs[dim] - rad, gs[dim] + rad)
            sample.append(s)
        return np.asarray(sample)
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        """
        # FILL in your code here
        new_sample = neighbor.state + self.step_size * (sample - neighbor.state) / np.linalg.norm(sample - neighbor.state)
